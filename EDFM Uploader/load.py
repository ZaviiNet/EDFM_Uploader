...
PLUGIN_VERSION = "1.4.0"  # Incremented for enhanced logging
...
CONSTRUCTION_API_URL = "https://edfm.space/api/v1/construction_update"
COMMANDER_STATUS_API_URL = "https://edfm.space/api/v1/commander_status"
MARKET_DATA_API_URL = "https://edfm.space/api/v1/market_data"
...
last_known_market_data = {}  # type: ignore


class ConfigDialog:
    def __init__(self, parent_frame_for_tab, config_obj):
        plugin_logger.debug(f"[{PLUGIN_NAME}] ConfigDialog.__init__ called.")
        self.parent_frame = parent_frame_for_tab  # This is the tab_frame
        self.config = config_obj
        self.status_label_widget = None  # Renamed to avoid conflict with any 'status_label' variable

        # Configure the parent frame's grid column to expand
        self.parent_frame.columnconfigure(0, weight=1)

        try:
            # API Key Configuration
            api_frame = ttk.LabelFrame(self.parent_frame, text="API Configuration", padding="10")
            # Use grid for direct children of parent_frame
            api_frame.grid(row=0, column=0, sticky="ew", padx=5, pady=5)
            plugin_logger.debug(f"[{PLUGIN_NAME}] ConfigDialog: api_frame created and gridded.")

            # Internal layout of api_frame can still use grid (it's a different parent)
            ...
            plugin_logger.debug(f"[{PLUGIN_NAME}] ConfigDialog: API key section widgets gridded into api_frame.")

            # Data Sending Preferences
            prefs_frame = ttk.LabelFrame(self.parent_frame, text="Data Sending Preferences", padding="10")
            prefs_frame.grid(row=1, column=0, sticky="ew", padx=5, pady=10)  # Use grid
            plugin_logger.debug(f"[{PLUGIN_NAME}] ConfigDialog: prefs_frame gridded.")

            # Internal layout of prefs_frame can use pack
            self.send_construction_var = tk.BooleanVar(
                value=self.config.get_bool(CONFIG_SEND_CONSTRUCTION_DATA, default=True))
            ttk.Checkbutton(prefs_frame, text="Send Colonisation Construction Data",
            ...
            plugin_logger.debug(f"[{PLUGIN_NAME}] ConfigDialog: Preference checks packed into prefs_frame.")
            ...
            info_frame.grid(row=2, column=0, sticky="ew", padx=5, pady=10)  # Use grid
            plugin_logger.debug(f"[{PLUGIN_NAME}] ConfigDialog: info_frame gridded.")

            # Internal layout of info_frame can use pack
            ...
            galnet_link.bind("<Button-1>", lambda e: webbrowser.open_new_tab(GALNET_URL))
            ...
            construction_link.bind("<Button-1>",
                                   lambda e: webbrowser.open_new_tab(GALNET_URL.replace("/galnet", "/construction")))

            ttk.Label(info_frame, text="To get an API key, signup on the fleetmanager app.").pack(
                pady=(10, 0), anchor="w")
            api_link = ttk.Label(info_frame, text="Get an API-Key", foreground="blue", cursor="hand2")
            api_link.pack(pady=(0, 5), anchor="w")
            api_link.bind("<Button-1>",
                          lambda e: webbrowser.open_new_tab(GALNET_URL.replace("/galnet", "/login")))
            ttk.Button(info_frame, text="Save Debug Log", command=self._save_debug_log).pack(pady=(10, 5), anchor="w")
            plugin_logger.debug(f"[{PLUGIN_NAME}] ConfigDialog: Info links and debug button packed into info_frame.")

            # Save Button
            save_button = ttk.Button(self.parent_frame, text="Save Settings", command=self._save_config)
            save_button.grid(row=3, column=0, pady=20)  # Use grid
            plugin_logger.debug(f"[{PLUGIN_NAME}] ConfigDialog: Save button gridded.")
            ...
            # Use grid for the error label as well
            ...
            self.parent_frame.rowconfigure(0, weight=1)  # Make error label expand if it's the only thing
    ...
    def _save_config(self):
        try:
            ...
            self._show_status_message("Settings saved!", "green")
        except Exception as e:
    ...
    def _show_status_message(self, message, color="green", duration=3000):
        ...
        # Find the save button to grid before it
        save_button = None
        for child in self.parent_frame.winfo_children():
            if isinstance(child, ttk.Button) and child.cget("text") == "Save Settings":
                save_button = child
                break
        ...
        if save_button:
            current_max_row = self.parent_frame.grid_size()[1]  # (cols, rows)
            status_row = current_max_row
            if save_button.grid_info():
                status_row = int(save_button.grid_info().get("row", current_max_row - 1)) + 1

            self.status_label_widget.grid(row=status_row, column=0, pady=2, sticky="ew")
        else:
            self.status_label_widget.grid(row=10, column=0, pady=2, sticky="ew")

        self.status_label_widget.after(duration,
    ...
    def _save_debug_log(self):
        ...
        edmc_log_path = None
        # Attempt to find the main EDMC log file path
        main_edmc_logger = logging.getLogger("EDMC")
        if not main_edmc_logger.handlers:
            main_edmc_logger = logging.getLogger()

        for handler in main_edmc_logger.handlers:
            if isinstance(handler, logging.FileHandler):
                edmc_log_path = handler.baseFilename
                plugin_logger.info(f"Found EDMC log file via handler: {edmc_log_path}")
                break

        if not edmc_log_path:
            ...
            for log_name in possible_log_names:
                ...
                if os.path.exists(potential_path):
                    edmc_log_path = potential_path
                    plugin_logger.warning(f"Found EDMC log file via fallback path: {edmc_log_path}")
                    break
            if not edmc_log_path:
                plugin_logger.error(f"Could not determine EDMC log file path. Checked common locations in {temp_dir}.")
        ...
        save_path = tkinter.filedialog.asksaveasfilename(initialfile=default_filename, defaultextension=".log",
                                                         filetypes=[("Log files", "*.log"), ("Text files", "*.txt"),
                                                                    ("All files", "*.*")])
        if not save_path: plugin_logger.info("Save debug log cancelled by user."); return
        try:
            with open(save_path, 'w', encoding='utf-8') as f_out:
                ...
                with open(edmc_log_path, 'r', encoding='utf-8', errors='replace') as f_in: shutil.copyfileobj(f_in,
                                                                                                              f_out)
            plugin_logger.info(f"Debug log saved successfully to: {save_path}")
            self._show_status_message(f"Debug log saved to:\n{os.path.basename(save_path)}", "green", duration=5000)
        except Exception as e:
...
def send_data_worker():
    ...
    while True:
        ...
        try:
            ...
            if target_url is None and data_type == "ShutdownSignal":
                plugin_logger.info(f"Send worker thread: Stop signal received.");
                break
            ...
            for attempt in range(MAX_RETRIES):
                try:
                    log_identifier_parts = [data_type]
                    if payload and isinstance(payload, dict):
                        if 'event_type' in payload:
                            log_identifier_parts.append(f"Event: {payload['event_type']}")
                        elif 'event' in payload:
                            log_identifier_parts.append(f"Event: {payload['event']}")
                        if 'MarketID' in payload: log_identifier_parts.append(f"MktID: {payload['MarketID']}")

                    log_identifier = ", ".join(filter(None, log_identifier_parts)) or "UnknownEvent"
            ...
            pl_info = str(payload)[:100] if payload else "N/A"
            ...
            if 'event_queue' in locals() and hasattr(event_queue, 'task_done'):
                event_queue.task_done()
...
def plugin_stop():
    ...
    if worker_thread and worker_thread.is_alive():
        event_queue.put((None, None, "ShutdownSignal", None))
...
def journal_entry(cmdr, is_beta, system, station, entry, state):
    ...
    if event_name in ['TechnologyBroker', 'ColonisationConstructionDepot'] and this_config.get_bool(
            CONFIG_SEND_CONSTRUCTION_DATA, default=True):

        # *** ADDED LOGGING FOR THE RAW 'entry' OBJECT ***
        ...
        # *** ADDED LOGGING FOR THE FINAL 'payload' TO BE SENT ***
        plugin_logger.info(
        ...
        try:
            ...
            # Original log line below is now covered by the more detailed one above
            # plugin_logger.info(f"Queued ConstructionUpdate for MarketID {payload.get('MarketID')} from {event_name}")
        except Exception as e:
        ...
        try:
            ...
            if current_market_commodities != last_known_market_data.get(market_id):
                market_payload = {
                    ...
                    "timestamp": market_file_data.get("timestamp", timestamp),
                    "MarketID": market_id,
    ...
    # Handle commander status updates
        ...
        if event_name == 'LoadGame':
            ...
            payload = {**base_payload,
                       ...
                       "ship": entry.get('Ship'),
                       "shipID": entry.get('ShipID'),
                       "shipName": entry.get('ShipName'),
                       "shipIdent": entry.get('ShipIdent'),
            ...
            payload = {**base_payload,
                       ...
                       "starPos": entry.get('StarPos'),
                       "jumpDist": entry.get('JumpDist'),
                       "fuelUsed": entry.get('FuelUsed'),
                       "fuelLevel": entry.get('FuelLevel')}
            ...
            payload = {**base_payload,
                       ...
                       "SRV": entry.get('SRV', False),
                       "Taxi": entry.get('Taxi', False),
                       "Multicrew": entry.get('Multicrew', False),
                       "OnStation": entry.get('OnStation', False),
                       "OnPlanet": entry.get('OnPlanet', False),
                       "StationName": entry.get('StationName'),
                       "StationType": entry.get('StationType'),
                       "Body": entry.get('Body'),
                       "BodyID": entry.get('BodyID'),
        ...
        if payload and data_type_log_name:
            try:
                event_queue.put((COMMANDER_STATUS_API_URL, api_key, f"CommanderStatus_{data_type_log_name}",
                                 payload))
        ...
        cargo_trigger_events = ['Cargo', 'LoadGame', 'Docked', 'Undocked',
                                'MarketBuy', 'MarketSell', 'MiningRefined',
                                'CollectCargo', 'EjectCargo', 'MissionCompleted',
        ...
        if event_name in cargo_trigger_events:
            if event_name == 'Cargo' and 'Inventory' in entry:
                ...
                journal_dir_path_str = this_config.get_str(
                    "journaldir") or this_config.default_journal_dir
        ...
        if send_cargo_update:
            ...
            if inventory_to_process != last_known_cargo:
                cargo_payload = {"event_type": "CargoUpdate", "timestamp": timestamp, "Inventory": inventory_to_process}
                try:
                    event_queue.put((COMMANDER_STATUS_API_URL, api_key, "CommanderStatus_CargoUpdate",
                                     cargo_payload))
